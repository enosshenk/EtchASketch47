...
#include "EASCanvasRenderTarget2D.h"

UEASCanvasRenderTarget2D::UEASCanvasRenderTarget2D(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
	bNeedsTwoCopies = true;
}

UCanvasRenderTarget2D* UEASCanvasRenderTarget2D::EASCreateCanvasRenderTarget2D(UObject* WorldContextObject, TSubclassOf<UCanvasRenderTarget2D> CanvasRenderTarget2DClass, int32 Width, int32 Height)
{
	if ((Width > 0) && (Height > 0) && (CanvasRenderTarget2DClass != NULL))
	{
		UEASCanvasRenderTarget2D* NewCanvasRenderTarget = ConstructObject<UEASCanvasRenderTarget2D>(CanvasRenderTarget2DClass, GetTransientPackage());
		...
		//	NewCanvasRenderTarget->InitAutoFormat(Width, Height);
			NewCanvasRenderTarget->InitCustomFormat(Width, Height, EPixelFormat::PF_FloatRGB, true);
...
void UEASCanvasRenderTarget2D::UpdateResource()
{
	// Call parent implementation
	Super::Super::UpdateResource();
